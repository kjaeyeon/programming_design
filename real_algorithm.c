...
while (1) {
    int nx 다음 이동할 위치를 나타내는 변수 = 현재 플레이어의 위치를 나타내는 변수x + 상하좌우 방향으로 움직일 때의 x축 변화량 저장하는 배열;
    int ny 다음 이동할 위치를 나타내는 변수 = 현재 플레이어의 위치를 나타내는 변수y + 상하좌우 방향으로 움직일 때의 y축 변화량 저장하는 배열;
...
void gold_item() {
    ...
    //선택한 위치에 황금열쇠 생성
    map[item_width][item_length] = 2;
}//2는 황금열쇠로 출력 맵에서 0은 길, 1은 벽이다.
